<?php
/**
 * IELTS Academic Diagnostic Test — session launcher.
 * Same DB-driven mock session system as the Full Mock tests (see
 * ielts_full_mock_001.php), just an abridged test registered under
 * IELTS_ACA_DIAGNOSTIC in mock_test_map.php.
 */
...
$stmt = $db->prepare("SELECT id, code FROM tests WHERE code = 'IELTS_ACA_DIAGNOSTIC' AND is_active = 1 LIMIT 1");
$stmt->execute();
