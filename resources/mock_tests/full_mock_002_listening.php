...
// Mock 2 — audio and asset paths
$mockCode      = $session['mock_code']; // e.g. IELTS_GT_MOCK02
    ...
    <main class="content p-3">
        ...
        <div class="sticky-header">
            ...
            <div style="background:#1e1b4b;color:#c7d2fe;padding:.6rem 1.25rem;border-radius:8px;margin-bottom:.5rem;display:flex;align-items:center;gap:1.5rem;font-size:.82rem;font-weight:600;">
                ...
                <a href="full_mock_002_listening.php?session_id=<?= $session_id ?>" style="color:#c7d2fe;text-decoration:none;border-bottom:2px solid #6366f1;padding-bottom:2px;">🎧 Listening</a>
                <a href="full_mock_002_reading.php?session_id=<?= $session_id ?>"   style="color:#a5b4fc;text-decoration:none;">📖 Reading</a>
        ...
        <div class="section-content" style="padding-top:<?= $isAdmin ? '110px' : '60px' ?>;">
            ...
            <div class="part-panel <?= $partNum === 1 ? 'active' : '' ?>"
                ...
                // Mock 2 Part 2 is a Food Hall (no map needed).
                // Add entries here if future parts require a diagram image.
                $partImages = [
                    // e.g. 3 => ['url' => ACADEMY_URL . 'assets/img/...', 'alt' => '...', 'caption' => '...']
                ];
                ...
                <div class="worksheet-img-wrap">
                    ...
                    <div class="worksheet-img-caption"><?= htmlspecialchars($img['caption']) ?></div>
                </div>
                <?php endif; ?>
                ...
                foreach ($partQuestions as $q):
                    ...
                    // Open a new instruction block when instructions change
                ...
                <?php
                    // Stimulus heading (table title, form name, etc.)
                ...
                <?php elseif ($qtype === 'matching'): ?>
                    <?php
                    // Show the key box once per instruction group, using the stimulus_text
                    // as the box title (pulled from DB — no hardcoding needed).
                    if (!$matchBoxShown && !empty($qopts)):
                        $matchBoxShown = true;
                    ?>
                    <div class="match-box">
                        <div class="match-box-title"><?= htmlspecialchars($q['stimulus_text'] ?? 'Options') ?></div>
                        <div class="match-box-grid">
                            <?php foreach ($qopts as $opt): ?>
                            <div>
                                <span class="match-key"><?= htmlspecialchars($opt['option_label']) ?></span>
                                &nbsp;<?= htmlspecialchars($opt['option_text']) ?>
                            </div>
                            <?php endforeach; ?>
                        </div>
