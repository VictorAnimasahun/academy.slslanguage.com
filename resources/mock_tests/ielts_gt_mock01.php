<?php
/**
 * IELTS GT Mock Test 1 — session launcher.
 * Routes through the DB-driven mock session system (full_mock_001_listening.php).
 * The old hardcoded content has been replaced by migrations 015–018.
 */
...
$stmt = $db->prepare("SELECT id, code FROM tests WHERE code = 'IELTS_FULL_MOCK_001' AND is_active = 1 LIMIT 1");
$stmt->execute();
$test = $stmt->fetch(PDO::FETCH_ASSOC);

if (!$test) {
    die("Mock Test 1 is not configured yet. Please run database migrations 015–018 and contact your instructor.");
}
...
if ($existing) {
    ...
    if (is_null($existing['listening_attempt_id'])) {
        header("Location: full_mock_001_listening.php?session_id={$sid}");
    } elseif (is_null($existing['reading_attempt_id'])) {
        header("Location: full_mock_001_reading.php?session_id={$sid}");
...
header("Location: full_mock_001_listening.php?session_id={$session_id}");
exit();
