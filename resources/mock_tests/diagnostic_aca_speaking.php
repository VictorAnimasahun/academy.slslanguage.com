<?php
/**
 * IELTS Academic Diagnostic — Speaking.
 * Self-contained: pure status/handoff screen (Speaking Part 2 is administered
 * live by the instructor, not online). Does NOT share mock_speaking.php — that
 * page hardcodes redirects to the Full Mock 1 section files, which would send
 * a diagnostic student to the wrong test if they land here before finishing
...
if (!$isAdmin && $session['status'] === 'in_progress') {
    ...
    if (is_null($session['writing_attempt_id'])) {
        $file = $map[$session['mock_code']]['writing']['file'] ?? 'diagnostic_aca_writing.php';
        header("Location: {$file}?session_id={$session_id}"); exit();
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $session['status'] === 'in_progress') {
    try {
        ...
        error_log('diagnostic_aca_speaking.php: ' . $e->getMessage());
    ...
    <div class="main-wrapper flex-grow-1">
        ...
        <main class="content p-3">
            ...
            <div class="panel">
                ...
                <span class="section-badge mb-3 d-inline-block">Diagnostic Complete</span>
                <h2 class="h4 fw-bold mt-3">You're all done with the written test!</h2>
                <p class="text-muted mt-2 mb-4">
                    An instructor will speak with you directly to complete your <strong>Speaking</strong> assessment
                    during the Masterclass. Once that's done, your full diagnostic results — showing where you stand
                    before Week 2 — will be released on your dashboard.
                </p>
            ...
            <div class="panel">
                ...
                <span class="section-badge mb-3 d-inline-block">Speaking</span>
                <h2 class="h4 fw-bold mt-3">One section remaining — speak to your instructor</h2>
                <p class="text-muted mt-2 mb-4">
                    The Speaking section of this diagnostic isn't done online. Your instructor will speak with you
