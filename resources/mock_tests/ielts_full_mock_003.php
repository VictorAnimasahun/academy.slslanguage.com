...
$stmt = $db->prepare("SELECT id, code FROM tests WHERE code = 'IELTS_FULL_MOCK_003' AND is_active = 1 LIMIT 1");
$stmt->execute();
...
if ($existing) {
    ...
    if (is_null($existing['listening_attempt_id'])) {
        header("Location: full_mock_003_listening.php?session_id={$sid}");
    } elseif (is_null($existing['reading_attempt_id'])) {
        header("Location: full_mock_003_reading.php?session_id={$sid}");
...
header("Location: full_mock_003_listening.php?session_id={$session_id}");
exit();
