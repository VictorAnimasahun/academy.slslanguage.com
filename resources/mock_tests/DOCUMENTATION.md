...
| `mock_speaking.php` | B | Submission/collation page, not an actual test — packages L/R/W results for instructor review; speaking itself is administered live and graded later in `sls-admin`. **IELTS only** — see the CELPIP exception below |
| `celpip_full_mock_speaking.php` | B | **CELPIP-only** online recorded Speaking, added 2026-09-12. `mock_test_map.php`'s `CELPIP_FULL_MOCK_A`/`B` entries point their `speaking` section here instead of at `mock_speaking.php`. Generic across Test A/B (task content picked by `$session['mock_code']`, same convention as the CELPIP listening/reading files below). Records all 8 tasks via `MediaRecorder`, uploads to `academy/api/speaking_upload.php` (`mock_session_id` set so recordings tie back to this session), transcribes via Groq Whisper, then on submit flips `mock_sessions.status` to `awaiting_speaking_grade` and emails the admin — same side effect `mock_speaking.php` has always had. Recordings/transcripts are graded in sls-admin's Speaking Recordings panel (`student_view.php`), not in this file. Full pipeline details: project memory `project_celpip_speaking_audio_pipeline.md`. |
| `celpip_full_mock_listening.php` / `celpip_full_mock_reading.php` | B | CELPIP Full Mock A/B Listening/Reading, generic across both tests via `$session['mock_code']`. Content seeded by migration 073. Not otherwise documented in this file (this doc predates the CELPIP work and is written IELTS-first) |
...
One row per attempt. Columns `listening_attempt_id`, `reading_attempt_id`, `writing_attempt_id` start NULL and are filled in as each section completes — this is how every page in the chain knows what to do next (resume vs. redirect vs. block out-of-order access). `status` moves `in_progress` → `awaiting_speaking_grade` (set by `mock_speaking.php` for IELTS, `celpip_full_mock_speaking.php` for CELPIP) → (something instructor-set after speaking is graded). There is no `speaking_attempt_id` column — Speaking is always graded manually onto `speaking_band`/`speaking_notes`, whether or not it was recorded online. Full release-to-student flow (speaking grading, banding, email/message notification) lives in `sls-admin/mock_session_detail.php`, documented in that project's own docs.

## Results delivery: what the student receives (and the tutor's preview)

Written after a student's mock result arrived with the instructor's analysis cut off (2026-09-25). Rules that came out of it:

- **Nothing the tutor wrote is ever capped.** The student gets the same text three ways, in full: the results page (`resources/practice_tests/my_results.php`), the PDF report, and the release email (`send_mock_results_email()` in `sls-admin/mock_session_detail.php`). Text sources: `mock_sessions.writing_notes` (tutor), `writing_ai_feedback` (AI, shown only if the AI grade succeeded), `speaking_notes` (tutor), and per-task `speaking_recordings.manual_score` / `manual_analysis` (latest recording per task).
- **One PDF builder.** `assets/js/mock_report_pdf.js` decides what the report contains (`composeMockReport`) and draws it (`buildMockReportPDF`). It is used by My Results, by the dashboard's "PDF" button, and by the tutor's preview. jsPDF neither wraps nor paginates; text past the bottom edge is silently clipped, so all free text goes through `para()`, which wraps and starts a new page per line. **Never add a fixed-height box or `.slice(0, n)` on tutor text.** Helvetica only has Latin-1 glyphs, so `pdfSafe()` maps or drops anything else (arrows, ticks, emoji).
- **A failed AI grade is never shown to a student.** `gradeMockEssay()` retries once and, on failure, returns `failed` (the saved feedback starts with `[AI GRADING FAILED]`, band 0). Students never see that text; the tutor's preview flags it as an error so a silent band 0 can't be released.
- **Tutor preview ("Preview what the student receives")** exists on `sls-admin/mock_session_detail.php` (mock) and `sls-admin/practice_session_detail.php` (practice speaking). It shows the results page, the actual PDF (mock) and the email, runs a checklist (missing scores, unscored tasks, missing comments, unsaved edits, AI failure), and includes anything typed in the forms but not yet saved, warning that only saved values reach the student. The mock preview loads `mock_report_pdf.js` from the academy site, so deploy academy before sls-admin.
- **CELPIP score display (instructor, 2026-09-25):** CELPIP levels are whole numbers and the overall is only an estimate. Skill scores show as whole numbers ("8"), and a half-point overall shows as the two levels either side (8.5 -> "8-9"; a whole overall stays "8"). IELTS keeps one decimal. Implemented by `mock_fmt_score()` (`includes/mock_report.php`), `fmtCelpip()` (`assets/js/mock_report_pdf.js`) and `sls-admin/mock_score_fmt.php`; use them, never a bare `number_format()`, when printing a mock score.
- **Correcting a student's score (tutor, 2026-09-25):** sls-admin can mark one Listening/Reading question right or wrong ("Mark correct / Mark wrong" on the row), or set an attempt's raw score and/or band by hand ("Correct this score"), on `mock_session_detail.php` (mock) and `student_view.php` test history (mock and practice attempts). Logic: `sls-admin/attempt_scoring.php` (band tables copied from `mock_save_section.php` — keep them in step), endpoint `attempt_correct_api.php`, every change logged to `score_corrections` (migration 125). A question fix re-adds the raw score and re-derives the band; if the attempt belongs to a released mock, the overall is recalculated (a pending session gets its overall at release as usual). A total set by hand leaves the per-question marks alone. Writing/Speaking are corrected through their own band forms. The tutor is then offered "Email corrected results" (mock page). Fixing a wrong answer KEY (e.g. migration 124) does not rescore saved attempts — use this feature per student.
- **Parity:** the per-task score/analysis editing and the score-from-tasks helper exist on BOTH the practice page and the mock page. Change one, change the other.
