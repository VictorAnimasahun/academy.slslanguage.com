 ...
 * Routes through the DB-driven mock session system (full_mock_004_listening.php).
 * PLACEHOLDER — no content migrations exist yet. Follow the FM2 checklist in
 * resources/mock_tests/DOCUMENTATION.md to seed IELTS_FULL_MOCK_004 and its sections.
 */
...
$stmt = $db->prepare("SELECT id, code FROM tests WHERE code = 'IELTS_FULL_MOCK_004' AND is_active = 1 LIMIT 1");
$stmt->execute();
$test = $stmt->fetch(PDO::FETCH_ASSOC);

if (!$test) {
    die("Mock Test 3 is not configured yet. Please run the database migrations for this mock and contact your instructor.");
}
...
if ($existing) {
    ...
    if (is_null($existing['listening_attempt_id'])) {
        header("Location: full_mock_004_listening.php?session_id={$sid}");
    } elseif (is_null($existing['reading_attempt_id'])) {
        header("Location: full_mock_004_reading.php?session_id={$sid}");
...
header("Location: full_mock_004_listening.php?session_id={$session_id}");
exit();
