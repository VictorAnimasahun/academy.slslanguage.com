...
$stmt = $db->prepare("SELECT * FROM courses WHERE folder_name = 'IELTS_Aca_1Mo' LIMIT 1");
$stmt->execute();
        ...
        <div class="course-card">
            ...
            <div class="highlight-box">
                <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                    ...
                    <div class="d-flex gap-2 align-items-center">
                        ...
                        <?php endif; ?>
                        <a href="<?= ACADEMY_URL ?>courses/IELTS_Aca_Crash/intro.php?from=IELTS_Aca_1Mo" class="btn btn-outline-primary btn-sm">
            ...
            <div class="content-section">
                ...
                <?php foreach ($modules as $month_num => $module): ?>
                    <?php $color = $month_colors[$month_num] ?? '#0b77ff'; $collapse_id = 'month' . $month_num; ?>
                    <div class="accordion-item mb-2" style="border-radius:10px;overflow:hidden;border:none;box-shadow:0 2px 8px rgba(0,0,0,0.06);">
                        <h2 class="accordion-header">
                            <button class="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target="#<?= $collapse_id ?>"
                                    style="background:<?= $color ?>18;font-weight:700;color:<?= $color ?>;">
                                <i class="bi bi-calendar3 me-2"></i>
                                <?= htmlspecialchars($module['title']) ?>
                                <span class="badge ms-auto me-2" style="background:<?= $color ?>;color:white;"><?= count($module['lessons']) ?> classes</span>
                            </button>
                        </h2>
                                ...
                                <li class="d-flex align-items-center justify-content-between px-3 py-2 <?= $idx < count($module['lessons'])-1 ? 'border-bottom' : '' ?>"
                                    ...
                                    <div class="d-flex align-items-center gap-2">
                                        <?php if ($can_access && $file_path): ?>
                                            <a href="<?= ACADEMY_URL . htmlspecialchars($file_path) ?>?from=IELTS_Aca_1Mo" class="text-decoration-none text-dark d-flex align-items-center gap-2">
                                        ...
                                        <?php else: ?>
                                            <span class="text-muted d-flex align-items-center gap-2">
                                                ...
                                                <span><strong>Class <?= $global_class ?>:</strong> <?= htmlspecialchars($lesson['title']) ?></span>
                                            </span>
                                        <?php endif; ?>
                                    </div>
            ...
            <div class="content-section">
                ...
                <div class="info-grid">
                    <div class="info-card"><h4><i class="bi bi-play-circle me-2"></i>Lesson Videos</h4><p class="mb-0">Targeted IELTS Academic video lessons covering all four skills — Reading, Writing, Listening, and Speaking.</p></div>
                    <div class="info-card"><h4><i class="bi bi-question-circle me-2"></i>Class Quizzes</h4><p class="mb-0">A consolidation quiz after every class to reinforce Academic strategies and band descriptor criteria.</p></div>
                    <div class="info-card"><h4><i class="bi bi-house-heart me-2"></i>Take-Home Tasks</h4><p class="mb-0">One practical task per class — Academic Reading summaries, Task 1 graph descriptions, or Task 2 essay drafts.</p></div>
                    <div class="info-card"><h4><i class="bi bi-clipboard-check me-2"></i>1 Practice Test Set</h4><p class="mb-0">A full IELTS Academic practice test across all four skills, spread throughout the teaching classes.</p></div>
                    <div class="info-card"><h4><i class="bi bi-journal-richtext me-2"></i>1 Full Mock Exam</h4><p class="mb-0">End-of-month full timed mock exam with written feedback and an estimated band score report.</p></div>
                    <div class="info-card"><h4><i class="bi bi-arrow-up-circle me-2"></i>Upgrade Anytime</h4><p class="mb-0">Upgrade to the 2-Month or 3-Month IELTS Academic plan at any time — your progress carries over.</p></div>
                </div>
            </div>

            <div class="action-buttons">
                <a href="<?= ACADEMY_URL ?>courses/IELTS_Aca_Crash/intro.php?from=IELTS_Aca_1Mo" class="btn btn-primary btn-lg">
